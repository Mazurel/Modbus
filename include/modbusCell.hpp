...
namespace MB {
/**
 * @brief Class that represents single modbus cell, it is either coil(bool) or reg(uint16_t).
 */
    class ModbusCell {
    private:
    ...
    public:
         ...
         */
        bool &coil() {
            if (isReg()) _value = static_cast<bool>(std::get<uint16_t>(_value));
         ...
         */
        uint16_t &reg() {
            if (isCoil()) _value = static_cast<uint16_t >(std::get<bool>(_value));
         ...
         */
        [[nodiscard]] const bool &coil() const
        {
            return std::get<bool>(_value);
        };
