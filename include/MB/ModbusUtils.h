...
/// @brief Checks if error code is modbus standard error code
inline bool is_standard_error_code(MBErrorCode code)
{
    switch (code) {
...
/// @brief All modbus standard function codes + Undefined one
...
/// @brief 根据 @see MBFunctionType检查“函数类型”
inline MBFunctionType function_type(const MBFunctionCode code)
{
    switch (code) {
...
/// @brief 获取基于函数代码的寄存器类型
inline MBFunctionRegisters function_register(const MBFunctionCode code)
{
    switch (code) {
        case ReadDiscreteOutputCoils:
        case WriteSingleDiscreteOutputCoil:
...
/// @brief Create uint16_t from buffer of two bytes, ex. { 0x01, 0x02 } => 0x0102
inline uint16_t big_endian_conv(const uint8_t *buf)
{
    return static_cast<uint16_t>(buf[1]) + (static_cast<uint16_t>(buf[0]) << 8);  // NOLINT(hicpp-signed-bitwise)
}

/// @brief 计算CRC
inline uint16_t calculate_crc(const uint8_t *buff, size_t len)
{
    static const uint16_t w_crc_table[] = {
    ...
    uint8_t n_temp;
    uint16_t w_crc_word = 0xFFFF;

    while (len--) {
        n_temp = *buff++ ^ w_crc_word;
        w_crc_word >>= 8;
        w_crc_word ^= w_crc_table[n_temp];
    }
    return w_crc_word;
}

/// @brief 计算CRC
inline uint16_t calculate_crc(const std::vector<uint8_t> &buffer)
{
    return calculate_crc(buffer.data(), buffer.size());
}

/// @brief 将uint16_t以大端字节形式拆分为两个uint8_t
///
/// @code
/// auto [highByte, lowByte] = Utils::splitUint16(val);
/// @endcode
inline std::pair<uint8_t, uint8_t> split_uint16(const uint16_t val)
{
    return std::make_pair((val >> 8) & 0xFF, val & 0xFF);
}

/// @brief Insert uint16_t into buffer of uint8_t's. Preserve big endianess.
inline void push_uint16(std::vector<uint8_t> &buffer, const uint16_t val)
{
    auto [high, low] = split_uint16(val);
