...
ModbusResponse::ModbusResponse(uint8_t slaveId, utils::MBFunctionCode functionCode, uint16_t address,
                               uint16_t registersNumber, std::vector<ModbusCell> values) :
        _slaveID(slaveId), _functionCode(functionCode),
        _address(address), _registersNumber(registersNumber),
        _values(std::move(values))
{
    ...
    switch (functionRegisters())
    {
        ...
        case utils::InputContacts:
            std::for_each(_values.begin() , _values.end(), []( ModbusCell& cell ) -> void
            {
                cell.coil();
            });
            break;
...
std::vector<uint8_t> ModbusResponse::toRaw() const
{
    ...
    if (functionType() == utils::Read)
    {
        if (_values[0].isCoil())
        {
            result.push_back( (_registersNumber / 8) + (_registersNumber % 8 == 0 ? 0 : 1)); // number of bytes to follow
